package com.zero.server.explosion;
...
public class ZeroExplosion extends Explosion {
    ...
    public ZeroExplosion(World world, Entity entity, EntityPlayer player, InfoType type, double x, double y, double z, float size, boolean causesFire, boolean smoking, boolean breaksBlocks) {
        ...
        if (!ForgeEventFactory.onExplosionStart(world, this)) {
            ...
            for (EntityPlayer obj : world.playerEntities) {
                Zero.getPacketHandler().sendToPlayer(new SPacketExplosion(x, y, z, size, affectedBlockPositions, getPlayerKnockbackMap().get(obj)), (EntityPlayerMP) obj);
            }
        }
    }
    ...
    public void doExplosionA() {
        ...
        List<Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(this.explosive, new AxisAlignedBB(k1, i2, j2, l1, i1, j1));
            ...
            if (!entity.isImmuneToExplosions()) {
                ...
                if (d12 <= 1.0D) {
                    ...
                    if (d13 != 0.0D) {
                        ...
                        if (player != null) {
                            entity.attackEntityFrom(new EntityDamageSourceZero(type.id, explosive, player, type).setExplosion(), (float) ((int) ((d10 * d10 + d10) / 2.0D * 7.0D * (double) f3 + 1.0D)));
                        } else {
